...
由于要对节点指向的数据类型中index成员进行维护，不得不放弃复用性：容器使用```vector```，堆节点数据类型为树节点的指针```Vertex*```，每个树节点Vertex包含一个成员变量```minWeightArcToTree```（指向到tree权重最小的边，NULL表示无此边），和一个```heapNodeIndex```(对应堆中的节点位置，```heapArcToTree.size()```表示不在堆中)。
...
#### upBubble(index)、downBubble(index)

> p85

递归函数，递归调用自己。对某一个节点进行向上（下）冒泡操作，维护最小堆的性质。冒泡的同时要对节点指向的图节点中```heapNodeIndex```成员进行维护。

#### makeHeap()

从乱序的容器建立堆，使用downBubble对包含子节点的堆节点进行向下冒泡操作

#### pop()

将首节点的index成员更改为超过堆大小的数，再用末节点替换首节点，删除末节点，再对首节点使用downBubble
